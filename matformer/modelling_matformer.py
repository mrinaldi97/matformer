...
payload_script="""
# Auto-generated by matformer integration
try:
    ...
    print("Please install Matformer with `pip install matformer` before using this model.")   
"""
def _write_auto_import_script(repo_path: str):
    """
    Writes a small script to ensure users importing the model from the Hub
    get a clear message if Matformer is not installed.
    """
    script_path = os.path.join(repo_path, "auto_import_matformer.py")
    
    with open(script_path, "w", encoding="utf-8") as f:
        f.write(payload_script)
    logger.info(f"Added auto_import_matformer.py to {repo_path}")

# -----------------------------------------------------------------------------
# Override push_to_hub to include payload
# -----------------------------------------------------------------------------
def _patched_push_to_hub(self, *args, **kwargs):
    repo_path = kwargs.get("repo_path", ".")
    try:
        _write_auto_import_script(repo_path)
    except Exception as e:
        logger.warning(f"Could not write payload script: {e}")
    return super(self.__class__, self).push_to_hub(*args, **kwargs)

# Monkey-patch the push_to_hub of our base model so that any subclass inherits it
