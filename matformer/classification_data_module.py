...
class ClassificationDataModule(pl.LightningDataModule):
    def __init__(
        ...
        """
        Args:
            data_loader: class that handles data loading (default works on CSV/TSV/JSON/CONLL files)
    ...
    def setup(self, stage=None):
        ...
        if train_truncation or val_truncation:
            ...
            print(
                f"This caused the truncation of {train_truncation} in the train set and {val_truncation} in the validation set"
            )
            print(
                "If this is unwanted behaviour, consider modifying the value in the config file\n"
                + "-" * 30
            )
    ...
    def _create_batch_dict(self, input_ids_list, labels_list):
        ...
        padded_sequence = PaddedTensor(
            tensor=input_ids_tensor, padding_mask=padding_mask
        )
