...
class ClassificationTrainingDataLoader:
    ...
    def _validate_and_load(self):
        ...
        if self.hf_dataset:
            try:
                dataset = load_dataset(
                    self.hf_dataset, name=self.hf_config, split=self.hf_split
                )
                self.df = dataset.to_pandas()
            except Exception as e:
                raise ValueError(
                    f"Failed to load HF dataset '{self.hf_dataset}': {e}"
                ) from e
        ...
        if missing:
            raise ValueError(
                f"Missing columns: {missing}. Available: {list(self.df.columns)}"
            )
    ...
    def _load_conll(self, is_conllu: bool):
        ...
        with open(self.filepath, "r", encoding="utf-8") as f:
            for line in f:
                ...
                if not line or line.startswith("#"):
                    if tokens:
                        sentences.append(
                            {self.text_column: tokens, self.label_column: labels}
                        )
                        tokens, labels = [], []
                    continue
        ...
        if tokens:
            sentences.append({self.text_column: tokens, self.label_column: labels})
    ...
    def __repr__(self) -> str:
        source = (
            f"hf={self.hf_dataset}" if self.hf_dataset else f"file={self.filepath.name}"
        )
