...
class LayerConfig:
    ...
    positional_encoding: Optional[
        Union[
            List[Literal["alibi", "rope", "sinusoidal", "nope", "learnable"]],
            Literal["alibi", "rope", "sinusoidal", "nope", "learnable"],
        ]
    ] = "alibi"
...
class BaseSubModelConfig:
    ...
    masked_substitution_rate: Optional[
        Union[float, Tuple[float, float], List[float]]
    ] = None  # fixed rate (float)/ rate range (Tuple/List[float, float] lower_bound,upper_bound )
...
class ModelConfig(BaseSubModelConfig):
    ...
    def _matches_pattern(self, layer_idx: int, pattern: str) -> bool:
        ...
        if pattern.startswith("every_"):
            ...
            start = next(
                (int(parts[i + 1]) for i, p in enumerate(parts) if p == "start"), 0
            )
            end = next(
                (int(parts[i + 1]) for i, p in enumerate(parts) if p == "end"),
                self.num_hidden_layers or 32,
            )

            return start <= layer_idx < end and (layer_idx % 2 == 1) == is_odd
...
def load_and_validate_classification_config_from_dict(
    ...
    extra_fields = {
        k: config_dict.pop(k)
        for k in ["training", "data", "save_dir", "wandb_project", "wandb_run_name"]
        if k in config_dict
    }
    ...
    print(
        f"Loaded: {config.num_hidden_layers}L x {config.hidden_size}d × {config.num_attention_heads}h, "
